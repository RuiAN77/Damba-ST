...
def cal_all_delay(data,filename):
    ...
    np.savetxt(f'../data/{filename}/delay.csv', all_delay, delimiter=',')
...
def check_delay(adj, all_delay, back, threshold):
    ...
    all_delay = torch.from_numpy(all_delay).to(torch.float32)
...
def load_st_dataset(dataset, args):
    ...
    # 5 / 1 / 2012 - 6 / 30 / 2012 Tuesday
        ...
        all_delay = cal_all_delay(data, dataset)
...
def define_dataloder(args):
    ...
    i=0
    for dataset_name in args.dataset_use:#
        print("this is domain", dataset_name)
        #dataloder_trn_list, dataloder_val_list, dataloder_tst_list = [], [], []
        data = load_st_dataset(dataset_name, args)
        num_nodes_dict[dataset_name] = data.shape[1]
        data_train, data_val, data_test = split_data_by_ratio(data, args.val_ratio, args.test_ratio)
    ...
    if (1-args.val_ratio-args.test_ratio) <= 0:
        ...
        train_dataloader = DataLoader(train_combine, batch_size=1, shuffle=False)
    if args.val_ratio <= 0:
        val_dataloader = None
    else:
        val_dataloader = DataLoader(val_combine, batch_size=1, shuffle=False)
