...
class Damba(nn.Module):
    def __init__(self, args, dataset_use, device, dim_in):
        ...
        self.mamba_blocks = nn.ModuleList([
            Mamba2(
                    d_model=self.embed_dim,  # Model dimension d_model
                    d_state=128,  # SSM state expansion factor #本来是128
                    A_init_range=(1, 1.1),
        ...
        ''''
        self.spa_vector1 = nn.Parameter(torch.randn(self.output_window))
        self.spa_vector2 = nn.Parameter(torch.randn(self.output_window))
        
        self.tempo_vector1 = nn.Parameter(torch.randn(self.output_window))
        self.tempo_vector2 = nn.Parameter(torch.randn(self.output_window))

        self.spatempo_vector1 = nn.Parameter(torch.randn(self.output_window))
        self.spatempo_vector2 = nn.Parameter(torch.randn(self.output_window))
        '''
    '''
    def orthogonalize(self):
        v1 = self.vector1
        v2 = self.vector2
        v2_orthogonal = v2 - (torch.dot(v1, v2) / torch.dot(v1, v1)) * v1
        self.vector2.data = v2_orthogonal
    '''
    ...
    def forward(self, input, lbls, select_dataset):
        ...
        path = self.walk_path_dict[select_dataset].to(self.device)#torch.Size([node_num, pathlen])
        pe_dict = self.walk_pe_dict[select_dataset].to(self.device)

        bs, patch_nums, num_nodes, num_feas = enc.shape

        '''
        projection_v1 = (torch.dot(enc, self.vector1) / torch.dot(self.vector1, self.vector1)) * self.vector1
        projection_v2 = (torch.dot(enc, self.vector2) / torch.dot(self.vector2, self.vector2)) * self.vector2
        enc = projection_v1 + projection_v2
        '''

        N, nodeselects = path.shape
        path = path.unsqueeze(0).expand(bs, -1, -1)
        batch_indices = torch.arange(bs).view(-1, 1, 1).expand(-1, N, nodeselects)  # (bs, N, nodeselects)
        spatio_enc = enc[batch_indices, :, path, :]  # (bs, N, nodeselects, time_steps, num_feas)

        '''
        spatio_enc=[]
        for i in range(enc.shape[0]): 
            for j in range(path.shape[0]):
                spatio_enc.append(enc[i][:,path[j],:])
        spatio_enc = torch.stack(spatio_enc)
        '''
